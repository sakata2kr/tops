/* jQuery Plugin jsPanel for bootstrap
   ...
   Copyright (c) 2014 Stefan Straßer, http://stefanstraesser.eu/
...
(function ( $ ) {
    ...
    $.fn.jsPanel = function( config , callback ) {
        ...
        // tagname des elements an das das jsPanel gehangt wird und window Daten
        ...
        // Counter fur die Panels im jeweils verwendeten Container (fur top & left)
        ...
        var jsPanel = $('<div class="jsPanel normalized" style="z-index:1000;display:none;">'+
                            '<nav class="jsPanel-navbar-hdr navbar navbar-default" role="navigation">'+
                                ...
                                '<ul class="p-icon">'+
                                    //'<li><a href="#none" onclick=d.fixed(this);><img src="../resources/img/icon/component_icon01.png" /></a></li>'+
                                    //'<li><a href="#none" onclick=d.center(this);><img src="../resources/img/icon/component_icon02.png"></a></li>'+
                                    '<li><a href="#none" onclick=showCase.proc(this,"favorites");><img src="../resources/img/icon/component_icon03.png"></a></li>'+
                                '</ul>'+
                                    '<ul class="nav navbar-nav navbar-right jsPanel-controls jsp-mini">'+
                                        ...
                                        '<li class="jsPanel-hdr-r-btn-close" onclick="panelRemove(this);"><a href="#" onclick="panelRemove(this);"><span class="glyphicon glyphicon-remove"></span></a></li>'+
                                    '</ul>'+
                                '</div><!-- /.navbar-collapse -->'+
         ...
         * und jsPanel ins document einfugen
         */
        ...
        if( option.modal )
        {
            ...
            // backdrop wieder einfugen
            $( 'body' ).append( backdrop );
            // jsPanel in backdrop einfugen
        ...
        {
            commons_MTA( option, jsPanel, 'closeonly' );
            // cursor zurucksetzen, weil draggable deaktiviert
        ...
        /* option.title | default: function - (Uberschrift) des Panels */
        ...
        /* font-awesome | bootstrap iconfonts einfugen wenn option.iconfont gesetzt */
        ...
        if( option.toolbarFooter )
        {
            ...
            // toolbar Elemente einfugen und konfigurieren
        ...
        /* option.draggable */
        // default-config fur draggable steht in $.fn.jsPanel.defaults.draggable
        ...
        /* option.resizable */
        // default-config fur resizable steht in $.fn.jsPanel.defaults.resizable
        ...
        /* option.content */
        // option.content kann auch eine Funktion (auch IIFE) oder ein jQuery Objekt sein, die den Inhalt zuruckliefert
        ...
        if( typeof option.autoclose == 'number' && option.autoclose > 0 )
        {
            ...
            window.setTimeout( function(){
                // func autoclose pruft erst ob es das el noch gibt
        ...
        /* der folgende code block ermoglicht 'center' fur top & left auch dann, wenn width und/oder height 'auto' sind */
        ...
        // 직전 scroll position 값 : 창 최대화시 top으로 scroll하는 기능 추가
        var lastScrollTop = 0;

        /*
         * DIE HANDLER FUR DAS PANEL
         *
         */
        ...
        if( option.controls.buttons != 'closeonly' && option.controls.buttons != false ){
            ...
            $('.jsPanel-hdr-r-btn-max', jsPanel).on('click', function(e){
                ...
                /*
                // 현재 scroll position 값 : 창 최대화시 top으로 scroll하는 기능 추가
                var windowPosTop = getScrollTop();

                if(windowPosTop == 0) {
                    $("html, body").animate({ scrollTop: lastScrollTop+"px" });
        ...
        // 현재 scroll position 값 얻는 함수 : 창 최대화시 top으로 scroll하는 기능 추가
         ...
         * ausreichend Funktionalitat bereitstellt, die hier verwendet werden kann
         *
         */
        ...
        jsPanel.title = function()
        {
            ...
            {
                // ohne Argument liefert die Funktion das Element das den Titletext enthalt
            ...
            {
                // in allen anderen Fallen liefert die Funktion das jsPanel an sich
                return this;
            }
        };
        ...
        {
            // das parent-Element des Panels ermitteln, wird benotigt, um dessen childpanels nach close zu repositionieren wenn minimized
        ...
        // childpanels des jspanels loschen
        ...
        jsPanel.minimize = function()
        {
            ...
            {
                // wenn der Container fur die minimierten jsPanels noch nicht existiert ->
            ...
            if( !this.hasClass( 'minimized' ) )
            {
                ...
                // counter der minimierten jsPanels und css left fur das zu minimierende Panel berechnen
                var count =  $( '.jsPanel.minimized' ).length,
                    left = ( ( count ) * 300 ) + 'px';
                // jsPanel bearbeiten
                //mokgoon
                this.alterClass( 'normalized maximized', 'minimized' )
                    .animate( { width:'300px' , height:'26px' , left:left , top:0 , opacity:1 , zIndex:1000 } );
        ...
        jsPanel.maximize = function()
        {
            if ( !this.hasClass('normalized') )
            {
                //## scrolltop 값이 0보다 큰 경우 처리를 위한 코드 : 일반크기로 조정되는 경우(최대창에서 최대화 버튼 다시 클릭시)
                $("html, body").animate({ scrollTop: lastScrollTop+"px" });
                window.scroll(0,lastScrollTop); // 창을 최대화했던 스크롤 위치로 돌아간다.
            ...
            {
                //## scrolltop 값이 0보다 큰 경우 처리를 위한 코드 : 최대화버튼 클릭시
                ...
                // maximale Große anwenden
                var width =  parseInt( this.parent().outerWidth() , 10 ) - 10 + 'px',
                    height = parseInt( this.parent().outerHeight() , 10 ) - 10 + 'px';

                if( par == 'body' )
                {
                    //## 최대화 수정(dashboard 상단 메뉴바는 보이게)
                    this.animate( {left: winscrollLeft()+5+'px' , top: winscrollTop()+45+'px' , width: width , height: winouterHeight()-50+'px' } );
                }
                else
                {
                    // sonst
        ...
        {
            // der callback bekommt das jsPanel als Argument 'jsPanel' ubergeben
            callback( jsPanel );
        }
     ...
     *
     */
    // Funktion bildet einen Wert fur css z-index
    function set_zi(){
        var zi = 0;
    ...
    function configToolbar( optionModal, optionToolbar, cssToolbar, panel, rtl ){
        ...
        else if( $.isArray( optionToolbar ) )
        {
            for( i = 0; i < optionToolbar.length; i++ )
            {
                ...
                {
                    var el = $( optionToolbar[i].item ),                        // Anderung in 1.3.0
                        type = el.prop('tagName');                              // Anderung in 1.3.0
                    if( typeof optionModal === 'string' && type == 'BUTTON' )   // Anderung in 1.3.0
                    {
                        // set text of button
    ...
    // wird in option.autoclose verwendet und pruft vor Anwendung von .close() ob es das jsPanel uberhaupt noch gibt
    function autoclose( jsPanel, id ){
        var elmt = $('#' + id );
        if( elmt ){
            elmt.fadeOut('slow', function(){
                jsPanel.close(); // elmt geht hier nicht weil .close() nicht fur elmt definiert ist
            });
        }
    }
